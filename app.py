...
@app.route('/getAllWishes',methods=['POST'])
def getAllWishes():
    try:
        #if session.get('user'):
        _limit = dashLimit
        ...
        conn = mysql.connect()
        cursor = conn.cursor()
        cursor.callproc('sp_GetAllWishes',(_limit,_offset,_total_records))
        ...
        cursor = conn.cursor()
        cursor.execute('SELECT @_sp_GetAllWishes_2');
...
@app.route('/getAllWishesSigned',methods=['POST'])
def getAllWishesSigned():
    try:
        if session.get('user'):
            _limit = dashLimit
            _offset = request.form['offset']
